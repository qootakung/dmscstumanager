...
import { ArrowLeft, Printer, BookMarked, Save } from 'lucide-react';
...
// 5 indicators for Reading / Analytical Thinking / Writing (MOE)
export const READING_INDICATORS = [
  'อ่านหนังสือได้คล่อง จับใจความสำคัญของเรื่องที่อ่านได้',
  'สามารถคิดวิเคราะห์ แยกแยะข้อเท็จจริง ข้อคิดเห็นได้',
  'สามารถสรุปเนื้อหา เชื่อมโยงความรู้ และแสดงความคิดเห็นได้',
  'สามารถเขียนสื่อความ ถ่ายทอดความรู้ ความคิดได้ตรงตามจุดประสงค์',
  'ใช้กระบวนการอ่าน คิดวิเคราะห์ และเขียน เพื่อการเรียนรู้และแก้ปัญหา',
];

const SHORT_INDICATORS = ['อ่านคล่อง', 'คิดวิเคราะห์', 'สรุป/เชื่อมโยง', 'เขียนสื่อความ', 'ใช้กระบวนการ'];

// MOE conversion: sum 0-15 → level 0-3 → label
export const readingLevel = (sum: number): { level: number; label: string } => {
  if (sum >= 14) return { level: 3, label: 'ดีเยี่ยม' };
  if (sum >= 10) return { level: 2, label: 'ดี' };
  if (sum >= 6) return { level: 1, label: 'ผ่าน' };
  return { level: 0, label: 'ไม่ผ่าน' };
};

type Scores = Record<string, number[]>; // studentId -> 5 scores
...
const ReadingAnalysisAssessment: React.FC<Props> = ({
  ...
  const storageKey = `pp5-reading-analysis-${currentGrade}-${selectedAcademicYear}-${selectedSemester}`;
  ...
  const updateScore = (sid: string, idx: number, val: number) => {
    setScores(prev => {
      const row = prev[sid] ? [...prev[sid]] : new Array(5).fill(0);
  ...
  const rows = useMemo(() => students.map(s => {
    const sid = s.studentId || s.id;
    const row = scores[sid] || new Array(5).fill(0);
    const sum = row.reduce((a, b) => a + (Number(b) || 0), 0);
    const { level, label } = readingLevel(sum);
  ...
  const handlePrint = () => {
    ...
    win.document.title = `การประเมินการอ่าน คิดวิเคราะห์ และเขียน ${currentGrade}`;
    const el = win.document.createElement('div');
    win.document.body.appendChild(el);
    createRoot(el).render(
      <ReadingAnalysisPrintable
        rows={rows}
        grade={currentGrade}
  ...
  return (
    <div className="space-y-4">
      <Card className="border-0 shadow-lg bg-gradient-to-r from-purple-600 to-fuchsia-700">
      ...
      <Card>
        <CardContent className="pt-4 space-y-4">
          <div className="flex flex-wrap items-end gap-4">
            ...
            <Button onClick={handlePrint} className="bg-fuchsia-700 hover:bg-fuchsia-800"><Printer className="w-4 h-4 mr-1" />พิมพ์</Button>
          </div>

          <div className="flex flex-wrap gap-3 text-sm bg-fuchsia-50 border border-fuchsia-200 rounded p-3">
            <span className="font-semibold text-fuchsia-800">เกณฑ์การประเมิน (คะแนนเต็ม 15):</span>
            <span>14–15 = ดีเยี่ยม (3)</span>
            <span>10–13 = ดี (2)</span>
            <span>6–9 = ผ่าน (1)</span>
            <span>0–5 = ไม่ผ่าน (0)</span>
          </div>
              ...
              <table className="w-full border-collapse text-sm">
                <thead>
                  <tr className="bg-fuchsia-100">
                    ...
                    {SHORT_INDICATORS.map((t, i) => (
                      <th key={i} className="border border-gray-400 p-1 w-24" title={READING_INDICATORS[i]}>
                ...
                <tbody>
                  {rows.map((r, idx) => (
                    <tr key={r.student.id} className="hover:bg-fuchsia-50/50">
                ...
                <tfoot>
                  <tr className="bg-fuchsia-50 font-semibold">
                    <td className="border border-gray-400 p-2 text-right" colSpan={8}>สรุปจำนวนนักเรียน</td>
...
interface PrintableProps {
  rows: Array<{ student: Student; sid: string; row: number[]; sum: number; level: number; label: string }>;
  grade: string;
  semester: string;
  academicYear: string;
  teacherName: string;
...
const ReadingAnalysisPrintable: React.FC<PrintableProps> = ({ rows, grade, semester, academicYear, teacherName, summary }) => {
  ...
  return (
    <div style={{ fontFamily: "'TH SarabunPSK', 'TH Sarabun', 'Sarabun', sans-serif", padding: '6mm' }}>
      <style>{`
        ...
        .pp5-ra-table { width: 100%; border-collapse: collapse; font-size: 14pt; }
        .pp5-ra-table th, .pp5-ra-table td { border: 1px solid #000; padding: 2px 4px; }
        .pp5-ra-table th { background-color: #fdf2fb; text-align: center; vertical-align: middle; }
        .pp5-ra-table td.num { text-align: center; }
        .rot { writing-mode: vertical-rl; transform: rotate(180deg); white-space: nowrap; padding: 4px 2px; font-size: 12pt; height: 130px; }
      `}</style>

      <div style={{ textAlign: 'center', marginBottom: '6px' }}>
        <div style={{ fontSize: '18pt', fontWeight: 'bold' }}>แบบสรุปการประเมินการอ่าน คิดวิเคราะห์ และเขียน</div>
        <div style={{ fontSize: '16pt' }}>
          โรงเรียนบ้านดอนมูล ชั้นประถมศึกษาปีที่ {grade.replace('ป.', '')} ภาคเรียนที่ {semester} ปีการศึกษา {academicYear}
        </div>
      </div>

      <table className="pp5-ra-table">
        <thead>
          <tr>
            <th rowSpan={2} style={{ width: '32px' }}>ที่</th>
            <th rowSpan={2} style={{ minWidth: '180px' }}>ชื่อ - นามสกุล</th>
            <th colSpan={5}>ตัวชี้วัด (กรอกตัวเลขระดับ 0-3)</th>
          ...
          <tr>
            {READING_INDICATORS.map((t, i) => (
              <th key={i} style={{ width: '42px' }}>
      ...
      <div style={{ marginTop: '8px', fontSize: '14pt' }}>
        ...
        <div style={{ marginTop: '2px', fontSize: '12pt', color: '#444' }}>
          <strong>เกณฑ์:</strong> 14–15 = ดีเยี่ยม (3), 10–13 = ดี (2), 6–9 = ผ่าน (1), 0–5 = ไม่ผ่าน (0)
        </div>
      </div>
...
export default ReadingAnalysisAssessment;