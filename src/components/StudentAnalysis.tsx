...
const StudentAnalysis: React.FC = () => {
  ...
  // จำลองข้อมูลคะแนนสำหรับนักเรียน
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-50 p-6">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="bg-white/90 backdrop-blur-sm shadow-2xl border-0 rounded-3xl overflow-hidden">
          ...
          <div className="p-6">
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              <TabsList className="grid w-full grid-cols-4 bg-gradient-to-r from-purple-50 to-blue-50 rounded-2xl p-2 border border-purple-100">
                <TabsTrigger 
                  value="overview" 
                    ...
                    <CardContent className="p-6">
                      <ResponsiveContainer width="100%" height={300}>
                        <BarChart data={barChartData}>
                          ...
                          <YAxis />
                          <Tooltip />
                          <Bar dataKey="average" fill="url(#gradient)" />
                          <defs>
              ...
              <TabsContent value="comparison" className="mt-6">
                <Card className="shadow-lg border-0">
                  <CardHeader className="bg-gradient-to-r from-indigo-500 to-purple-500 text-white rounded-t-lg">
                    <CardTitle className="flex items-center gap-2">
                      <BarChart3 className="w-5 h-5" />
                      เปรียบเทียบผลการเรียนระหว่างวิชา
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="p-6">
                ...
                <Card className="shadow-lg border-0">
                  <CardHeader className="bg-gradient-to-r from-slate-600 to-gray-700 text-white rounded-t-lg">
                    <CardTitle className="flex items-center gap-2">
                      ...
                      สรุปรายงานการวิเคราะห์
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="p-6">
                    <div className="space-y-6">
                      <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-6">
                        <h3 className="text-lg font-semibold text-gray-800 mb-4">สรุปผลการวิเคราะห์วิชา {selectedSubject}</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            <p className="text-sm text-gray-600">จำนวนนักเรียนทั้งหมด</p>
                            <p className="text-2xl font-bold text-blue-600">{analysisData.total} คน</p>
                          </div>
                          <div>
                            <p className="text-sm text-gray-600">คะแนนเฉลี่ย</p>
                            <p className="text-2xl font-bold text-purple-600">{Math.round(analysisData.averageScore * 100) / 100} คะแนน</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="bg-green-50 rounded-lg p-4 border-l-4 border-green-500">
                          <h4 className="font-semibold text-green-800">กลุ่มเก่ง</h4>
                          <p className="text-green-600">{analysisData.excellent} คน ({analysisData.total > 0 ? Math.round((analysisData.excellent / analysisData.total) * 100) : 0}%)</p>
                          <p className="text-xs text-green-500 mt-1">คะแนน 80 ขึ้นไป</p>
                        </div>
                        <div className="bg-orange-50 rounded-lg p-4 border-l-4 border-orange-500">
                          <h4 className="font-semibold text-orange-800">กลุ่มปานกลาง</h4>
                          <p className="text-orange-600">{analysisData.average} คน ({analysisData.total > 0 ? Math.round((analysisData.average / analysisData.total) * 100) : 0}%)</p>
                          <p className="text-xs text-orange-500 mt-1">คะแนน 70-79</p>
                        </div>
                        <div className="bg-red-50 rounded-lg p-4 border-l-4 border-red-500">
                          <h4 className="font-semibold text-red-800">กลุ่มอ่อน</h4>
                          <p className="text-red-600">{analysisData.needsImprovement} คน ({analysisData.total > 0 ? Math.round((analysisData.needsImprovement / analysisData.total) * 100) : 0}%)</p>
                          <p className="text-xs text-red-500 mt-1">คะแนนต่ำกว่า 70</p>
                        </div>
                      </div>

                      <div className="bg-gray-50 rounded-lg p-4">
                        <h4 className="font-semibold text-gray-800 mb-2">ข้อเสนอแนะ</h4>
                        <ul className="space-y-2 text-sm text-gray-600">
                          {analysisData.needsImprovement > analysisData.total * 0.3 && (
                            <li className="flex items-start gap-2">
                              <span className="text-red-500">•</span>
                              กลุ่มอ่อนมีสัดส่วนสูง ควรให้ความใส่ใจเพิ่มเติม
                            </li>
                          )}
                          {analysisData.excellent > analysisData.total * 0.5 && (
                            <li className="flex items-start gap-2">
                              <span className="text-green-500">•</span>
                              ผลการเรียนดีมาก ควรส่งเสริมให้พัฒนาต่อ
                            </li>
                          )}
                          <li className="flex items-start gap-2">
                            <span className="text-blue-500">•</span>
                            ควรจัดกิจกรรมเสริมสำหรับกลุ่มที่ต้องการการพัฒนา
                          </li>
                        </ul>
                      </div>
                    </div>
        ...
        <div className="flex justify-center">
          <div className="flex items-center gap-6 text-4xl opacity-20">
